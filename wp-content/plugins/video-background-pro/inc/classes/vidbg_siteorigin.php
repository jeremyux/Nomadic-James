...
if ( ! class_exists( 'Vidbg_SiteOrigin' ) ) {
	...
	class Vidbg_SiteOrigin {
		...
		/**
		 * The group name used within SiteOrigin to organize fields.
		 *
		...
		/**
		 * The [vidbg] shortcode attributes holder.
		 *
		 * @var Array $vidbg_atts
		 */
		protected $vidbg_atts;

		/**
		 * Setup our SiteOrigin integration.
		 */
		 ...
		 *
		 * @since 2.7.0
		 * @param Array   $groups The SiteOrigin groups.
		 * @param Integer $post_id The Post ID.
		 * @param Array   $args Not sure.
		 */
		 ...
		 *
		 * @since 2.7.0
		 * @param Mixed $siteorigin_row_atts The row attributes.
		 */
		 ...
		 *
		 * @since 2.7.0
		 * @param String $output The code that is outputted before the SiteOrigin row.
		 * @param Array  $grid_item The Grid item.
		 * @param Array  $grid_attributes The grid attributes.
		 */
		public function generate_shortcode_before_row( $output, $grid_item, $grid_attributes ) {
			...
			if ( function_exists( 'vidbgpro_install_plugin' ) ) {
				...
				if ( 'youtube' === $this->vidbg_atts['type'] && empty( $this->vidbg_atts['youtube_url'] ) ) {
					// Use var_dump( 'no youtube video exists' ); to debug.
					return;
				}
				...
				if ( 'self-host' === $this->vidbg_atts['type'] && empty( $this->vidbg_atts['mp4'] ) && empty( $this->vidbg_atts['webm'] ) ) {
					// use var_dump( 'no self hosted video exists' ); to debug.
					return;
				}
			} else {
			...
			if ( array_key_exists( 'loop', $this->vidbg_atts ) ) {
				$this->vidbg_atts['loop'] = true === $this->vidbg_atts['loop'] ? 'false' : 'true';
			}
			...
			if ( array_key_exists( 'end_frame_poster', $this->vidbg_atts ) ) {
				$this->vidbg_atts['end_frame_poster'] = true === $this->vidbg_atts['end_frame_poster'] ? 'true' : 'false';
			}

			if ( array_key_exists( 'frontend_play_button', $this->vidbg_atts ) ) {
				$this->vidbg_atts['frontend_play_button'] = true === $this->vidbg_atts['frontend_play_button'] ? 'true' : 'false';
			}

			if ( array_key_exists( 'frontend_volume_button', $this->vidbg_atts ) ) {
				$this->vidbg_atts['frontend_volume_button'] = true === $this->vidbg_atts['frontend_volume_button'] ? 'true' : 'false';
			}

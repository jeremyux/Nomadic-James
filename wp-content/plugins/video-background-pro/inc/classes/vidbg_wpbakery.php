...
if ( ! class_exists( 'Vidbg_WPBakery' ) ) {
	...
	class Vidbg_WPBakery {
		...
		/**
		 * The field prefix that is appended to the beginning of all fields.
		 *
		 * @var String $prefix The field prefix.
		 */
		private $prefix;

		/**
		 * Holds the [vidbg] shortcode attributes.
		 *
		 * @var Array $vidbg_atts The shortcode attributes.
		 */
		protected $vidbg_atts;

		/**
		 * Set up the WPBakery Integration.
		 *
		 * @param Mixed $vc_row_atts The Visual Composer Row attributes.
		 */
		...
		public function generate_shortcode() {
			...
			// Use var_dump( $this->vc_row_atts ); to debug.
			if ( array_key_exists( 'loop', $this->vidbg_atts ) ) {
				$this->vidbg_atts['loop'] = 'false' === $this->vidbg_atts['loop'] ? 'true' : 'false';
			}
	...
	if ( ! function_exists( 'vc_theme_before_vc_row' ) ) {
		 ...
		 *
		 * @since 2.7.0
		 * @param Array $atts The new attributes.
